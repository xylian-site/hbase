...
public class HFilePrettyPrinter extends Configured implements Tool {
  ...
  static final long[] RANGES = new long[] { 1, 3, 10, 50, 100, 500, 1_000, 5_000, 10_000, 50_000, 100_000, 500_000, 750_000, 1_000_000, 5_000_000, 10_000_000, 50_000_000, 100_000_000};
  ...
  private static class KeyValueStatsCollector {
    ...
    private final SimpleReporter simpleReporter = SimpleReporter.newBuilder()
      .outputTo(new PrintStream(metricsOutput))
      .addStats(keyLen)
      .addStats(valLen)
      .addStats(rowSizeBytes)
      .addStats(rowSizeCols)
      .build();
  ...
  private static class SimpleReporter {
    ...
    private SimpleReporter(PrintStream output,
                            List<KeyValueStats> stats,
                            Locale locale,
                            TimeZone timeZone) {
      this.output = output;
      this.stats = stats;
    ...
    private void printHistogram(KeyValueStats stats) {
      ...
      if (stats.hasRangeCounts()) {
        ...
        // print any remaining, likely 0 since our ranges are large, but it gives some context
